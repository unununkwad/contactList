  ...
  // Set error alert
    ...
    const editContact = () => {
        ...
        if (name === "" ||
            ...
            // send edit data to save
            axios({
                url: `${process.env.REACT_APP_API}Contacts/EditContact`,
                method: 'PUT',
                headers: {
    ...
    useEffect(() => {
        ...
        axios({
            ...
            setEmails(response.data.filter(a => a !== details.email));
        }).catch((error) => {
            console.log(error);
        });
    }, []);

    // set data to default data in inputs
    useEffect(() => {
        setName(details.name);
        setLastName(details.lastName);
        setEmail(details.email);
        setPassword(details.password);
        setSelectedCategory({label: details.category.name, value: details.category.id});
        setPhoneNumber(details.phoneNumber);

        if (details.category.id === 1) setTimeout(function () {setSelectedSubCategory({label: details.subCategory.name, value: details.subCategory.id})}, 200);
        else if (details.category.id === 3) setTimeout(function (){ setSubCategory(details.subCategory.name)}, 200);
                ...
                <VuiBox mb={3}>
                    <Card>
                        <VuiBox display="flex" justifyContent="space-between" alignItems="center" mb="22px">
                            ...
                            <VuiBox>
                                <VuiButton color='warning' onClick={() => editContact()}>
                                    <IoIosSave />
                                    Zapisz
                                </VuiButton>
                                <VuiButton color='dark' onClick={() => (setEditContact(null), setShowDetails(details))}>
